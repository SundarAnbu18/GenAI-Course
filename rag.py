"""
my_rag.py — a personal RAG for Sundar Anbu, calling the Anthropic API.
    ...
    python my_rag.py ask "how should I introduce myself to a new client?"
    python my_rag.py chat
    python my_rag.py sources             # what's currently indexed
    python my_rag.py ask "..." --since 2026-06-01 --tag client
...
NOTES_DIR = "./notes"                  # drop .md / .txt files here, any structure
INDEX_PATH = ".rag_index.npz"          # cache; rebuilt automatically when files change
ENV_PATH = ".env"                      # ANTHROPIC_API_KEY lives here; never commit it
...
def main() -> None:
    ...
    if command == "ask":
        if len(args) < 2:
            sys.exit('Usage: python my_rag.py ask "your question" [--since DATE] [--tag TAG]')
